...
/obj/item/needle/proc/sew(mob/living/target, mob/living/user)
	...
	while(!QDELETED(target_wound) && !QDELETED(src) && \
		...
		if(target_wound.bleed_rate == 0 && !informed)
			patient.visible_message(span_smallgreen("One last drop of blood trickles from the [(target_wound.name)] on [patient]'s [affecting] before it closes."), span_smallgreen("The throbbing warmth coming out of [target_wound.name] soothes and stops. It no longer bleeds."))
			informed = TRUE
