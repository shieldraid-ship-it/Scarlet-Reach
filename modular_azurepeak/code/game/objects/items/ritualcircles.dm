...
/obj/structure/ritualcircle/zizo/proc/rituosbone(src)
	...
	spawn(20)
		...
		if(no_breasts)
			qdel(no_breasts)
		var/obj/item/bodypart/no_liver = target.getorganslot(ORGAN_SLOT_LIVER)
		if(no_liver)
			qdel(no_liver)
		var/obj/item/bodypart/no_heart = target.getorganslot(ORGAN_SLOT_HEART)
		if(no_heart)
			qdel(no_heart)
		var/obj/item/bodypart/no_lungs = target.getorganslot(ORGAN_SLOT_LUNGS)
		if(no_lungs)
			qdel(no_lungs)
		var/obj/item/bodypart/no_stomach = target.getorganslot(ORGAN_SLOT_STOMACH)
		if(no_stomach)
			qdel(no_stomach)
